module.exports = function() {
    ...
    function (roleName, energy) {
        ...
        if (global.global.buildingPlans[roleName] == undefined) {
            console.log("No building plans for " + roleName + " found!");
        }
        else if (global.global.buildingPlans[roleName][rcl - 1].minEnergy > energy && rcl > 1) {
            if (global.global.buildingPlans[roleName][rcl - 2].minEnergy > energy) {
                return null;
            }
            else {
                return global.global.buildingPlans[roleName][rcl - 2].body;
            }
        }
        else {
            return global.global.buildingPlans[roleName][rcl - 1].body;
        }
    };
