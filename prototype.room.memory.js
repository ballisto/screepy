...
Room.prototype.checkSegment = function() {
  if (this.memory.segment === undefined) {
    this.memory.segment = root.getNextSegmentId();
  }
};
...
Room.prototype.clearMemory = function() {
  ...
  for (const key of root.getSegmentKeys(this.memory.segment)) {
    if (key.startsWith(roomKeyPrefix)) {
      root.removeSegmentObject(this.memory.segment, roomKeyPrefix);
    }
  }
  this.memory = {
...
Room.prototype.getMemoryCostMatrix = function() {
  this.checkSegment();
  return root.getSegmentObject(this.memory.segment, this.getRoomMemorySegmentKey('costmatrix'));
};
...
Room.prototype.setMemoryCostMatrix = function(costMatrix) {
  this.checkSegment();
  root.setSegmentObject(this.memory.segment, this.getRoomMemorySegmentKey('costmatrix'), costMatrix, 'costmatrix');
};
